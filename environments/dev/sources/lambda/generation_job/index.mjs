...
export const handler = async (event) => {
  ...
  try {
    ...
    // ジョブ失敗時、DynamoDBにフラグを記録してトークン回復をトリガー
    console.error(`Job failed: workflowJobId=${workflowJobId}, error=${error.message}`)
    ...
    try {
      await ddb.send(new UpdateItemCommand({
        ...
        UpdateExpression: "SET job_failed = :true",
        ExpressionAttributeValues: {
          ":true": { BOOL: true },
        },
      }))
      console.log(`Marked job as failed in DynamoDB: workflowJobId=${workflowJobId}`)
    } catch (ddbError) {
      console.error(`Failed to mark job as failed in DynamoDB: ${ddbError.message}`)
    }
